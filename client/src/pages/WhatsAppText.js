...
export const Whatsapp = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const d = await axios.get('http://localhost:3002/Textdata');
      // const m = await axios.get('http://localhost:3002/prodata');
      // console.log('axios r');
      // console.log(d);
      setdata(d.data.Texts);
  ...
  const k = async (ids) => {
    ...
    console.log('axios ');
    const n = await axios.post('http://localhost:3002/Textdel', { id: ids });
    console.log('axios runing ');
  ...
  return (
    <>
      <Container maxWidth="lg" className=" max-h-[100px] scrollhost  flex flex-col drop-shadow-xl">
        <div className="  space-x-2 bg-white rounded-xl drop-shadow-xl py-4">
          {/* Plans */}
          ...
          <Edm
            ...
            cre={['Name', 'Description', 'edit']}
            dm={['name', 'text']}
          />

          {/* Products */}

          {/* <div className=" basis-1/2 flex flex-col space-y-5 pt-24 px-6">
            <p className=" text-3xl"> TextContent</p>
            <TextField multiline variant={'outlined'} rows={10} fullWidth className=" h-full " />
...
export const PlanPopUp = (props) => {
  ...
  const handleAdd = () => {
    const k = async () => {
      const d = await axios.post('http://localhost:3002/addText', data);
  ...
  return (
    <Dialog open={props.open} onClose={handleClose}>
      ...
      <DialogContent>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', paddingX: '10px' }}>
          <TextField
            onChange={handleName}
            sx={{ margin: '10px' }}
            autoFocus
